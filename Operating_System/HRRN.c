...
int dequeue(int time)
{
    ...
    {
	process_list[process_queue[i]].log.waiting_time = time - process_list[process_queue[i]].log.entry_time;
    }

    for(i = 0; i < queue_size - 1; i++){
        for(j = 0; j < queue_size - i - 1; j++){
		index1 = process_queue[j+1];
		index2 = process_queue[j+2];

		A = process_list[index1].log.waiting_time + process_list[index1].service_time / process_list[index1].service_time;
		B = process_list[index2].log.waiting_time + process_list[index2].service_time / process_list[index2].service_time;

                if(A > B)              
                {
                        temp = process_queue[j+1];        
                        process_queue[j+1] = process_queue[j+2];        
                        process_queue[j+2] = temp;   
		}
	}
    }
		
    queue_size --;
    return(process_queue[dequeue]);
...
int main(int argc, char *argv[])
{
    ...
    if (argc != 2) {
        printf("Usage: HRRN <process_file>\n");
        return(-1);
    }
    ...
    while(num_processed < n) { // until all processes are processed
        ...
        if (CPU_idle) {
            if (queue_size != 0) { // is queue not empty?
                current_process_index = dequeue(t);
                ...
                if (process_list[current_process_index].log.response_time == 0) {
                    t - process_list[current_process_index].log.entry_time;
                }
                CPU_idle = 0;
